...
export class AppComponent {
  ...
  @ViewChild('autoCompleteCounty') autoCompleteCounty: any;
  @ViewChild('autoCompleteTown') autoCompleteTown: any;
  ...
  constructor(private http: HttpClient) {
    http.get<Lists>(`${environment.serviceBase}/api/list.php`).subscribe(data => {
      ...
      this.counties.sort((a, b) => a.name.localeCompare(b.name));
      this.communities.sort((a, b) => a.name.localeCompare(b.name));
      this.towns.sort((a, b) => a.name.localeCompare(b.name));

      this.counties.forEach(element => {
        element.name = element.name.toLowerCase();
        element.name = element.name[0].toUpperCase() + element.name.slice(1).toLowerCase();

        if (element.name.includes(" ")) {
          var i = element.name.indexOf(" ");
          var letter = element.name[i + 1].toUpperCase();

          element.name = element.name.slice(0, i + 1) + letter + element.name.slice(i + 2);
        }

        this.countyDataSource.push(element)
      });

      this.communities.forEach(element => {
        element.name = element.name.toLowerCase();
        element.name = element.name.replace(this.unicodeWordMatch,
          function (txt) {
  ...
  focusCounty() {
    this.autoCompleteCounty.toggle(true);
  }
  ...
  townValueChange(value: any) {
    ...
    if (!entity)
      entity = this.communities.find(x => x.name == value);

    if (!this.autoCompleteCounty.value || this.autoCompleteCounty.value == "") {

      var county = this.counties.find(x => x.ID == entity?.countyID);
      this.autoCompleteCounty.value = county?.name;
    }
  ...
  townFilterChange(value: any) {
    let towns = this.towns.filter((s) => s.name.toLowerCase().indexOf(value.toLowerCase()) !== -1);
    let communities = this.communities.filter((s) => s.name.toLowerCase().indexOf(value.toLowerCase()) !== -1);
  ...
  focusTown() {
    this.autoCompleteTown.toggle(true);
  }
